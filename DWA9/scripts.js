...
class BookPreview extends HTMLElement {
  ...
  connectedCallback() {
      // Extract book data from attributes
      const bookId = this.getAttribute('data-id');
      const bookTitle = this.getAttribute('data-title');
      const bookImage = this.getAttribute('data-image');
      const bookAuthor = this.getAttribute('data-author');

      // Create HTML content for the book preview
      this.shadowRoot.innerHTML = `
          <style>
              /* Add any necessary styles for the component */
          </style>
          <div class="preview" data-id="${bookId}" data-preview>
              <img src="${bookImage}" alt="" class="preview__image">
              <div class="preview__info">
                  <h3 class="preview__title">${bookTitle}</h3>
                  <small class="preview__author">${bookAuthor}</small>
              </div>
          </div>
      `;
  }
